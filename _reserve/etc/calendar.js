function ShowCalendar() {
	...
	document.write("<table class='jscalendar' cellspacing='0' style='width:100%; max-width:520px; table-layout:fixed;'>");
	document.write("<tr class='cahead' style='height:64px;'>");
	if (isCurrentMonth) {
	...
	document.write("<tr style='height:44px;'>");
	for (i = 0; i < 7; i++) {
		document.write("<th class='cadayofweek' style='height:44px;'>");
	...
	for (i = 0; i < rowCount; i++) {
		document.write("<tr style='height:78px;'>");
		for (var j = 0; j < 7; j++) {
			if (j === 0) {
				document.write("<td class='casun' style='height:78px;'>");
			} else {
				document.write("<td style='height:78px;'>");
			}

			var cellDay = calendarCells[j + (i * 7)];
			if (cellDay === selectedDay) {
				document.write("<strong style='display:flex;align-items:center;justify-content:center;height:78px;'>", cellDay, "</strong>");
				...
				document.write("<strong class='canow' style='display:flex;align-items:center;justify-content:center;height:78px;'><a style='display:flex;align-items:center;justify-content:center;height:78px;width:100%;' href='Javascript:GoReserve(", displayYear, ",", displayMonth + 1, ",", cellDay, ",", schoolCode, ");'>", cellDay, "</a></strong>");
			} else if (cellDay !== "　") {
				document.write("<a style='display:flex;align-items:center;justify-content:center;height:78px;width:100%;' href='Javascript:GoReserve(", displayYear, ",", displayMonth + 1, ",", cellDay, ",", schoolCode, ");'>", cellDay, "</a>");
			} else {
				document.write(cellDay);
			}
			document.write("</td>");
