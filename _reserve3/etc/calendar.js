function ShowCalendar() {
	...
	if (isCurrentMonth) {
		...
		document.write("<th><a title='前月へ' href='Javascript:GoReserve(", previousMonthYear, ",", previousMonth + 1, ",", 32, ",\"", schoolCode, "\",\"", courseCode, "\");'>&laquo;</a></th>");
	}
	document.write("<th colspan='5' class='camonth'>");
	document.write(displayYear, "年", displayMonth + 1, "月");
	document.write("</th>");
	document.write("<th><a title='翌月へ' href='Javascript:GoReserve(", nextMonthYear, ",", nextMonth + 1, ",", 32, ",\"", schoolCode, "\",\"", courseCode, "\");'>&raquo;</a></th>");
	document.write("</tr>");
	...
	for (i = 0; i < rowCount; i++) {
		...
		for (var j = 0; j < 7; j++) {
			...
			if (cellDay === selectedDay) {
				...
				document.write("<strong class='canow' style='display:flex;align-items:center;justify-content:center;height:100px;'><a style='display:flex;align-items:center;justify-content:center;height:100px;width:100%;' href='Javascript:GoReserve(", displayYear, ",", displayMonth + 1, ",", cellDay, ",\"", schoolCode, "\",\"", courseCode, "\");'>", cellDay, "</a></strong>");
			} else if (cellDay !== "　") {
				document.write("<a style='display:flex;align-items:center;justify-content:center;height:100px;width:100%;' href='Javascript:GoReserve(", displayYear, ",", displayMonth + 1, ",", cellDay, ",\"", schoolCode, "\",\"", courseCode, "\");'>", cellDay, "</a>");
			} else {
				document.write(cellDay);
			}
			document.write("</td>");
...
function GoReserve(year, month, date, school, course) {
	if (date !== 32 && !course) {
		alert("コースを選択してください。");
		return;
	}
	...
	location.search = "Course=" + encodeURIComponent(course) + "&ClassYear=" + year + "&ClassMonth=" + month + "&ClassDate=" + date + "&School=" + encodeURIComponent(school);
}
